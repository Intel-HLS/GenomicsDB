...
$(GENOMICSDB_JAR): $(GENOMICSDB_JAVA_CLASS_FILES) $(GENOMICSDB_SHARED_LIBRARY)
	@cp -f $(GENOMICSDB_SHARED_LIBRARY) $(GENOMICSDB_JAVA_BUILD_DIRECTORY)/$(GENOMICSDB_JAVA_PKG_NAME)/libgenomicsdb.so
	cd $(GENOMICSDB_JAVA_BUILD_DIRECTORY) && jar cf $(GENOMICSDB_JAR_BUILD_BIN_RELATIVE)/$(GENOMICSDB_JAR_NAME) \
	    $(GENOMICSDB_JAVA_PKG_NAME)/*.class  $(GENOMICSDB_JAVA_PKG_NAME)/libgenomicsdb.so
